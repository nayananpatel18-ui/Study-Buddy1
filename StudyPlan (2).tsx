...
import { Clock, Coffee, Brain, CheckCircle2, AlertCircle, TrendingUp } from "lucide-react";
...
const StudyPlan = () => {
  ...
  const regeneratePlan = () => {
    ...
    if (tasksString && userMood) {
      const tasks: Task[] = JSON.parse(tasksString);
      const scheduled = generateSchedule(tasks, userMood);
      setScheduledTasks(scheduled);
      setShowTimer(false);
      setCurrentTaskIndex(null);
      ...
      toast({
        ...
        description: "Your study plan has been refreshed with a new schedule.",
      });
    }
  };
  ...
  const getProgressStats = () => {
    const studyTasks = scheduledTasks.filter(t => !isBreak(t));
    const completedStudyTasks = studyTasks.filter(t => t.id && completedTasks.has(t.id));
    const percentage = studyTasks.length > 0 ? (completedStudyTasks.length / studyTasks.length) * 100 : 0;
    return {
      total: studyTasks.length,
      completed: completedStudyTasks.length,
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          <div className="animate-fade-in flex items-center justify-between">
            ...
            {scheduledTasks.length > 0 && (
              <div className="flex gap-2">
                ...
                <Button onClick={regeneratePlan} variant="outline" size="lg">
                  Regenerate Plan
                </Button>
              </div>
            )}
          ...
          <Card className="glass-card animate-fade-in">
            <CardHeader>
              ...
              <CardDescription>
                Track your study session completion
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-muted-foreground">{getProgressStats().percentage}%</span>
                </div>
                <Progress value={getProgressStats().percentage} className="h-3" />
              </div>
              {getProgressStats().percentage === 100 && getProgressStats().total > 0 && (
                <div className="text-center p-3 rounded-lg bg-accent/20 border border-accent">
                  <p className="font-semibold text-accent">🎉 All tasks completed!</p>
                  <p className="text-sm text-muted-foreground mt-1">Amazing work! You've completed your study plan.</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <div className="space-y-4 animate-fade-in">
            {scheduledTasks.map((task, index) => {
              const isCompleted = task.id ? completedTasks.has(task.id) : false;
              return (
                <Card
                  key={index}
                  className={`glass-card glass-card-hover transition-all ${
                    isBreak(task) ? "border-accent/50" : ""
                  } ${isCompleted ? "opacity-60 bg-accent/5" : ""}`}
                >
                  <CardHeader>
                    <div className="flex items-start justify-between gap-4">
                      {!isBreak(task) && task.id && (
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <CardTitle className={`${isBreak(task) ? "text-accent" : ""} ${isCompleted ? "line-through" : ""}`}>
                            {task.name}
                          </CardTitle>
                          {isCompleted && (
